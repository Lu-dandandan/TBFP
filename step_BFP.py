...
#     med = np.nanmedian(data[idx], axis=1) # 一维矩阵
#     # 补充前后的数据点
#     first_values = med[0]
#     last_values = med[-1]
...
#     idx = idx.astype(np.int64)  # needed if oversampling_factor is not int
...
def sele_fitting_global(limit,window_mode): 
    # 返回函数
    if window_mode == 'fixed':
        if limit == 'no':
            return fun(fitting_global_fixed_window_no_limit)
        elif limit == 'yes':
            return fun(fitting_global_fixed_window_limit)
        else:
            raise ValueError("The limit inputted is out of range, Please input an int in ['no','yes']")
    elif window_mode == 'varied':
        if limit == 'no':
            return fun(fitting_global_varied_window_no_limit)
        elif limit == 'yes':
            return fun(fitting_global_varied_window_limit)
        else:
            raise ValueError("The limit inputted is out of range, Please input an int in ['no','yes']")
    else:   
        raise ValueError("The window_mode inputted is out of range, Please input an int in ['fixed','varied']")

def sele_result_fitting(limit):
    if limit == 'no':
        return fun(result_fitting_no_limit)
    else:
        return fun(result_fitting_limit)
...
@jit(nopython=True) 
def selection_fixed_window(phase,group,q,qmax,e,trend_order,P,cri0):
...
def selection_varied_window(phase,group,q,e,trend_order,P,cri0):
    # 只要transit里面至少有一个点，外面有trend+1个点 的段，而且这段没有gap, 否则扔掉该段。
...
#@jit(nopython=True)
def fitting_global_fixed_window_no_limit(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm,get_X_sig_cut,get_X_trend_cut,cri0,e,t,inv_df,wF,trend_order,sig_order):     
    # 这个d和tm0都是这个周期下最佳的
    ...
    if dlnLmax1 != dlnLmax1: # 不满足三段
        ...
        cut_index,group_cut,phase_cut,g_uni_final = selection_fixed_window(phase,group,q,qmax,e,trend_order,P,cri0)
...
#@jit(nopython=True)
def fitting_global_fixed_window_limit(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm,get_X_sig_cut,get_X_trend_cut,cri0,e,t,inv_df,wF,trend_order,sig_order):     
    # 这个d和tm0都是这个周期下最佳的
    d,tm0,dlnLmax1 = fold_map_tm0d(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm)
    if dlnLmax1 != dlnLmax1: # 不满足三段
        dlnLmax2 = np.nan
...
def fitting_global_varied_window_no_limit(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm,get_X_sig_cut,get_X_trend_cut,cri0,e,t,inv_df,wF,trend_order,sig_order):     
    # 这个d和tm0都是这个周期下最佳的
    d,tm0,dlnLmax1 = fold_map_tm0d(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm)
    if dlnLmax1 != dlnLmax1: # 不满足三段
        dlnLmax2 = np.nan
        ...
        phase,group = get_phase(t,tm0,P)
        cut_index,group_cut,phase_cut,g_uni_final = selection_varied_window(phase,group,q,e,trend_order,P,cri0)
...
#@jit(nopython=True)
def fitting_global_varied_window_limit(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm,get_X_sig_cut,get_X_trend_cut,cri0,e,t,inv_df,wF,trend_order,sig_order):     
    # 这个d和tm0都是这个周期下最佳的
    d,tm0,dlnLmax1 = fold_map_tm0d(P,d_sam,tm_sam,dlnL,tm_gap,OS_tm)
    if dlnLmax1 != dlnLmax1: # 不满足三段
        dlnLmax2 = np.nan
...
# 知道了最佳的P,tm0,d
#@jit(nopython=True)
def result_fitting_no_limit(window_mode,P,d,tm0,t,f,e,trend_order,cri0,get_X_sig_cut,get_X_trend_cut,inv_df,wF,sig_order):
# 出三个图：
...
# in_num,cut_num
    ...
    phase,group = get_phase(t,tm0,P)
    if window_mode == 'fixed': 
        cut_index,group_cut,phase_cut_ori,g_uni_final = selection_fixed_window(phase,group,q,qmax,e,trend_order,P,cri0)
    else:
        cut_index,group_cut,phase_cut_ori,g_uni_final = selection_varied_window(phase,group,q,e,trend_order,P,cri0)
    ...
    X_trend_cut = get_X_trend_cut(phase_cut.reshape(len(phase_cut),1),X0)
    inv_df_cut = inv_df[cut_index]
    X_cut,wX_cut = get_XwX(X_sig_cut,X_trend_cut,inv_df_cut)
    wF_cut = wF[cut_index]
...
def result_fitting_limit(window_mode,P,d,tm0,t,f,e,trend_order,cri0,get_X_sig_cut,get_X_trend_cut,inv_df,wF,sig_order):
# 出三个图：
# 1相位图：散点[phase_cut_group],[f_cut_group],拟合数据[model_folded_f_cut_group]
# 2去趋势相位图：散点phase_cut_ori_sort,de_f_cut,拟合数据model_folded_de_f_cut
# 3时间图：散点t,f,拟合数据[t_cut_group],[model_f_group]
...
# in_num,cut_num
    ...
    if sig_order == 4:
        if (L[0]/L[1]>-2) and (L[0]*L[1]<0): 
            ...
            X_cut,wX_cut = get_XwX(X_sig_cut2,X_trend_cut,inv_df_cut)
            covar = get_covar(wX_cut) 
