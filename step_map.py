...
def sele_fitting_2D(limit,window_mode): 
    # 返回函数
    if window_mode == 'fixed':
        if limit == 'no':
            return fun(fitting_2D_fixed_window_no_limit)
        elif limit == 'yes':
            return fun(fitting_2D_fixed_window_limit)
        else:
            raise ValueError("The limit inputted is out of range, Please input an int in ['no','yes']")
    elif window_mode == 'varied':
        if limit == 'no':
            return fun(fitting_2D_varied_window_no_limit)
        elif limit == 'yes':
            return fun(fitting_2D_varied_window_limit)
        else:
            raise ValueError("The limit inputted is out of range, Please input an int in ['no','yes']")
    else:
...
def fitting_2D_fixed_window_no_limit(i,seg_ts,seg_fs,seg_dfs,size_d,d_sam,tm_sam,e,
               sig_order,trend_order,get_X_sig_cut,get_X_trend_cut):
    seg_t0 = seg_ts[i]  
    seg_f0 = seg_fs[i]
    seg_df0 = seg_dfs[i]
    ...
    for k in range(size_d):
        ...
        for j in range(len(tm_sam_here)):
            ...
            if in_num < (sig_order/2.0+2):
                continue
            if left_num < trend_order+1:#np.maximum(trend_order+1,in_num*3):
                continue 
            if (cut_num-in_num-left_num) < trend_order+1:#np.maximum(trend_order+1,in_num*3):
                continue 
...
def fitting_2D_fixed_window_limit(i,seg_ts,seg_fs,seg_dfs,size_d,d_sam,tm_sam,e,
               sig_order,trend_order,get_X_sig_cut,get_X_trend_cut):
    seg_t0 = seg_ts[i]  
    seg_f0 = seg_fs[i]
    seg_df0 = seg_dfs[i]
...
def fitting_2D_varied_window_no_limit(i,seg_ts,seg_fs,seg_dfs,size_d,d_sam,tm_sam,e,
               sig_order,trend_order,get_X_sig_cut,get_X_trend_cut):
    seg_t0 = seg_ts[i]  
    seg_f0 = seg_fs[i]
    seg_df0 = seg_dfs[i]
...
def fitting_2D_varied_window_limit(i,seg_ts,seg_fs,seg_dfs,size_d,d_sam,tm_sam,e,
               sig_order,trend_order,get_X_sig_cut,get_X_trend_cut):
    seg_t0 = seg_ts[i]  
    seg_f0 = seg_fs[i]
    seg_df0 = seg_dfs[i]
